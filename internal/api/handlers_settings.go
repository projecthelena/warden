...
func validateOIDCSettings(values map[string]string) error {
	...
	if redirect := strings.TrimSpace(values["sso.oidc.redirect_url"]); redirect != "" {
		u, err := url.Parse(redirect)
		if err != nil {
			return fmt.Errorf("redirect URL for OIDC must be a path or HTTP(S) URL without credentials, query, or fragment")
		}
		validRelative := strings.HasPrefix(redirect, "/") && !strings.HasPrefix(redirect, "//") && u.Host == "" && u.RawQuery == "" && u.Fragment == ""
		validAbsolute := u.Host != "" && (u.Scheme == "https" || u.Scheme == "http") && u.User == nil && u.RawQuery == "" && u.Fragment == ""
		if !validRelative && !validAbsolute {
			return fmt.Errorf("redirect URL for OIDC must be a path or HTTP(S) URL without credentials, query, or fragment")
		}
	}
