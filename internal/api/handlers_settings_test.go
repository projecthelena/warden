...
func TestUpdateSettings_ValidatesOIDCBeforeSaving(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "scheme relative redirect", body: map[string]string{"sso.oidc.redirect_url": "//attacker.example/callback"}},
		{name: "redirect with query", body: map[string]string{"sso.oidc.redirect_url": "/callback?next=evil"}},
		{name: "absolute redirect with fragment", body: map[string]string{"sso.oidc.redirect_url": "https://warden.example/callback#fragment"}},
