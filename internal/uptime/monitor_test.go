...
func TestMonitor_RecordResult(t *testing.T) {
	...
	m := NewMonitor("m1", "http", "g1", "Test Monitor", "http://example.com", 60*time.Second, jobQueue, time.Now(), nil)
...
func TestMonitor_Getters(t *testing.T) {
	jobQueue := make(chan Job, 1)
	m := NewMonitor("m1", "http", "g1", "Monitor Name", "http://target.com", 10*time.Second, jobQueue, time.Now(), nil)
...
func TestMonitor_Scheduling(t *testing.T) {
	...
	// Short interval
	m := NewMonitor("m1", "http", "g1", "Fast", "http://fast.com", 10*time.Millisecond, jobQueue, time.Now(), nil)
...
func TestMonitor_AlignedScheduling(t *testing.T) {
	...
	m := NewMonitor("m1", "http", "g1", "Aligned", "http://aligned.com", interval, jobQueue, createdAt, nil)
...
func TestNewMonitor_ZeroCreatedAtDefaultsToNow(t *testing.T) {
	jobQueue := make(chan Job, 1)
	before := time.Now()
	m := NewMonitor("m1", "http", "g1", "Zero", "http://zero.com", 60*time.Second, jobQueue, time.Time{}, nil)
	after := time.Now()
...
func newTestMonitorWithConfig(cfg MonitorConfig) *Monitor {
	jq := make(chan Job, 1)
	m := NewMonitor("test", "http", "g1", "Test", "http://example.com", 60*time.Second, jq, time.Now(), nil)
	m.ApplyConfig(cfg)
	return m
}
